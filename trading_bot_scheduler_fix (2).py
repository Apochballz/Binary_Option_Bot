...
# STANDARD LIBS
# ==============================
import os, time, json, sqlite3, logging, asyncio, threading, socket, atexit
...
# ==============================
# DATA FETCH (CRASH-PROOF) - FIXED PRIORITY ORDER
# ==============================
@lru_cache(maxsize=100)
def fetch_ohlc(symbol, granularity="M1", count=500):
    log.info(f"🔍 FETCHING DATA for {symbol}")
    log.info(f"OANDA_KEY present: {bool(OANDA_API_KEY)}")
    log.info(f"TWELVE_KEY present: {bool(TWELVE_API_KEY)}")
    
    # TRY TWELVEDATA FIRST (Most reliable for forex)
    if TWELVE_API_KEY:
        try:
            twelve_symbol = symbol.replace("_", "/")
            log.info(f"📊 Fetching from TwelveData: {twelve_symbol}")
            
            r = requests.get("https://api.twelvedata.com/time_series", params={
                "symbol": twelve_symbol,
                "interval": "1min",
                "outputsize": min(count, 100),
                "apikey": TWELVE_API_KEY
            }, timeout=15)
            
            log.info(f"TwelveData Status: {r.status_code}")
            ...
            if r.status_code == 200:
                ...
                if "values" in data and data["values"]:
                    ...
                    log.info(f"✅ Got {len(df)} candles from TwelveData")
                    ...
                    for col in ['open', 'high', 'low', 'close']:
                        if col in df.columns:
                            ...
                            log.error(f"Missing column {col} in TwelveData response")
                            raise ValueError(f"Missing column {col}")
                    
                    # Handle volume - set default if missing
                    if 'volume' in df.columns:
                        df['volume'] = pd.to_numeric(df['volume'], errors='coerce').fillna(1000)
                    else:
                        log.info("Volume column missing, using default value 1000")
                        df['volume'] = 1000
                    
                    df = df.dropna(subset=['open', 'high', 'low', 'close'])
                    if len(df) > 10:
                        log.info(f"✅ TwelveData SUCCESS: Returning {len(df)} candles")
                        return df[['open', 'high', 'low', 'close', 'volume']].reset_index(drop=True)
                else:
                    log.warning(f"❌ TwelveData error: {data.get('message', 'No data')}")
            else:
                log.warning(f"❌ TwelveData failed: {r.text[:100]}")
        except Exception as e:
            log.warning(f"TwelveData fetch failed: {str(e)}")
    
    # TRY OANDA SECOND
    try:
        if OANDA_API_KEY:
            log.info("Trying OANDA...")
            ...
            if r.status_code == 200:
                ...
                if len(df) > 50:
                    log.info(f"✅ OANDA SUCCESS: Returning {len(df)} candles")
                    return df
    except Exception as e:
        log.warning(f"OANDA fetch failed: {e}")

    # YAHOO FINANCE LAST RESORT (ONE SIMPLE ATTEMPT)
    try:
        log.info("Trying Yahoo Finance as last resort...")
        yahoo_symbol = f"{symbol.replace('_', '')}=X"
        log.info(f"Yahoo symbol: {yahoo_symbol}")
        df = yf.download(yahoo_symbol, period="1d", interval="1m", progress=False, timeout=5)
        if len(df) > 10:
            log.info(f"✅ Yahoo success: {len(df)} candles")
            df = df.rename(columns=str.lower)
            df['volume'] = df.get('volume', 1000).fillna(1000)
            return df[['open', 'high', 'low', 'close', 'volume']].reset_index(drop=True)
    except Exception as e:
        log.warning(f"YFinance fetch failed: {str(e)[:100]}")
...
# ==============================
# SCHEDULER - SINGLETON PATTERN TO PREVENT DUPLICATES
# ==============================
...
def get_scheduler():
    ...
    with _scheduler_lock:
        if _scheduler_instance is None:
            ...
            # Only add jobs once
            _scheduler_instance.add_job(auto_scan_signals, 'interval', minutes=3, id='auto_scan', replace_existing=True)
            _scheduler_instance.add_job(retrain_ml_model, 'interval', hours=24, id='retrain_ml', replace_existing=True)
            # Register shutdown
            atexit.register(lambda: _scheduler_instance.shutdown() if _scheduler_instance and _scheduler_instance.running else None)
...
def is_primary_worker():
    """Check if this is the primary worker process - only one should run scheduler"""
    try:
        # Use process ID and environment to detect if we're the first worker
        pid = os.getpid()
        worker_id = os.environ.get('WERKZEUG_RUN_MAIN', '')
        
        # In Gunicorn, check if we can create a lock file
        lock_file = '/tmp/scheduler_primary.lock'
        ...
        if os.path.exists(lock_file):
            ...
            try:
                ...
                try:
                    os.kill(lock_pid, 0)
                    # Process exists, we're not primary
                    return False
                except OSError:
        ...
        try:
            with open(lock_file, 'w') as f:
                f.write(str(pid))
            log.info(f"✅ Primary worker detected (PID: {pid})")
            return True
        except:
            return False
    except Exception as e:
        log.warning(f"Worker detection failed: {e}, assuming primary")
        return True
...
# ==============================
# BACKGROUND SERVICES STARTER (CRASH-PROOF) - FIXED TO PREVENT DUPLICATES
# ==============================
def start_background_services():
    try:
        ...
        if not scheduler_instance.running:
            ...
            log.info("⏸️ Scheduler already running, skipping initialization")
    except Exception as e:
        log.exception("🔥 start_background_services crashed")

# ==============================
# GUNICORN INITIALIZATION - FIXED TO PREVENT DUPLICATE WORKERS
# ==============================
if os.environ.get("RAILWAY_ENVIRONMENT"):
    ...
    def init_for_gunicorn():
        try:
            time.sleep(2)
            # Only start scheduler in primary worker
            if is_primary_worker():
                log.info("✅ Primary worker - starting scheduler")
                start_background_services()
            else:
                log.info("⏸️ Secondary worker - scheduler disabled to prevent duplicates")
        except Exception as e:
