...
def fetch_ohlc(symbol, granularity="M1", count=500):
    ...
    try:
        log.info("Trying Yahoo Finance as fallback...")
        # Try multiple formats
        formats = [
            f"{symbol.replace('_', '')}=X",
            f"{symbol.replace('_', '')}",
            f"{symbol.replace('USD', '')}/USD"
        ]
        
        for fmt in formats:
            try:
                log.info(f"Trying Yahoo format: {fmt}")
                df = yf.download(fmt, period="2d", interval="1m", progress=False, timeout=10)
                if len(df) > 10:
                    log.info(f"✅ Yahoo success with {fmt}: {len(df)} candles")
                    df = df.rename(columns=str.lower)
                    df['volume'] = df.get('volume', 1000)
                    return df[['open', 'high', 'low', 'close', 'volume']].reset_index(drop=True)
            except Exception as e:
                log.info(f"Yahoo format {fmt} failed: {str(e)[:50]}")
                continue
    except Exception as e:
    ...
    try:
        if TWELVE_API_KEY:
            # CORRECT FORMAT: EUR/USD not EUR_USD
            ...
            r = requests.get("https://api.twelvedata.com/time_series", params={
                ...
                "interval": "1min",
                "outputsize": 100,  # Use 100 instead of count
