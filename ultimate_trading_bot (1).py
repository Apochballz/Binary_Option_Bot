...
FIXED FOR RAILWAY/GUNICORN DEPLOYMENT
"""
...
def start_background_services():
    """
    Initialize scheduler and background threads safely
    Prevents duplicate schedulers on Railway replicas
    """
    replica_id = os.environ.get("RAILWAY_REPLICA_ID", "primary")
    
    if replica_id == "primary" or not os.environ.get("RAILWAY_ENVIRONMENT"):
        scheduler.start()
        log.info("✅ Auto-signal scheduler started (Replica: %s)", replica_id)
        ...
        log.info("⏭️  Scheduler skipped on replica: %s", replica_id)
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=PORT, debug=False)supertrend', {}).get('trend') in ["BULLISH", "BEARISH"]:
        score += 5
